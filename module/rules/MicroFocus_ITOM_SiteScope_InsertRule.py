...
class ITOM_SiteScope():
    ...
    def log_error(self):
        """
        针对 MicroFocus ITOM SiteScope 中的 error.log 文件
        """
        # 初始化数据和相关控制参数
        logdata = []
        sqldata = []
            ...
            with open(self.filepath, mode='r', encoding='utf-8', errors='replace') as f:
                for line in f:
                    try:
                        ...
                        if isnoblk and isstart:
                            line = line.strip()
                            # 判断该行日志是否符合格式
                            if len(re.findall('\d{4}-\d{2}-\d{2} \d{2}:\d{2}:\d{2},\d{3}', line)) > 0:
                                log_level = line.split(' -')[0].strip().split(' ')[-1].strip()
                                log_time = sql_write.sqlite_to_datetime(line.split(' ', 2)[0] + ' ' + line.split(' ', 2)[1])
                                log_comp = line.split(' ', 2)[-1].split(' - ')[0][:-5].strip()
                                log_detail = line.split(' - ', 1)[-1].strip()
                ...
                for data in logdata:
                    if data.get('logdetail') == '':
                        data['logdetail'] = 'Null'
                    try:
                        sql_insert = 'INSERT INTO log_error (logfile, logline, loglevel, logtime, logcomp, logdetail) VALUES ("{}","{}","{}","{}","{}","{}");'.format(data.get('logfile'), str(data.get('logline')),data.get('loglevel'),data.get('logtime'),data.get('logcomp'),data.get('logdetail').replace('"',"'"))
